...
struct downlink_data {
    void send_telemetry(long data);  // The 1 minute health checks
};

// Commands for the QEYnet project
struct downlink_QEYnet {
    void response_ping();                // Response ping after receiving ping() from uplink
    void response_packet(long packet);    // Response packet after receiving send_packet() from uplink
};

// Commands for the ARO
struct downlink_aro {
    void send_selfie(long selfie);       // Send the selfie data along with other metrics
